    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reviews.update", [$review->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $review->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('rate') ? 'is-invalid' : '' }}" name="rate" id="rate">
                    ...
                    @foreach(App\Models\Review::RATE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('rate', $review->rate) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('rate'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service_type') ? 'is-invalid' : '' }}" name="service_type_id" id="service_type_id">
                    @foreach($service_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('service_type_id') ? old('service_type_id') : $review->service_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
