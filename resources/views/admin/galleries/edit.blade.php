    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.galleries.update", [$gallery->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="title">{{ trans('cruds.gallery.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $gallery->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $gallery->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.gallery.fields.upload_file_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="service_type_id">{{ trans('cruds.gallery.fields.service_type') }}</label>
                <select class="form-control select2 {{ $errors->has('service_type') ? 'is-invalid' : '' }}" name="service_type_id" id="service_type_id">
                    @foreach($service_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('service_type_id') ? old('service_type_id') : $gallery->service_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.gallery.fields.service_type_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    init: function () {
@if(isset($gallery) && $gallery->upload_file)
      var file = {!! json_encode($gallery->upload_file) !!}
