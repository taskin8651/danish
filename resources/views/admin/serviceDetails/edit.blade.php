...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.serviceDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.service-details.update", [$serviceDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="title">{{ trans('cruds.serviceDetail.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $serviceDetail->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('short_description') ? 'is-invalid' : '' }}" name="short_description" id="short_description">{{ old('short_description', $serviceDetail->short_description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $serviceDetail->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('projects') ? 'is-invalid' : '' }}" name="projects[]" id="projects" multiple>
                    @foreach($projects as $id => $project)
                        <option value="{{ $id }}" {{ (in_array($id, old('projects', [])) || $serviceDetail->projects->contains($id)) ? 'selected' : '' }}>{{ $project }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reviews') ? 'is-invalid' : '' }}" name="reviews[]" id="reviews" multiple>
                    @foreach($reviews as $id => $review)
                        <option value="{{ $id }}" {{ (in_array($id, old('reviews', [])) || $serviceDetail->reviews->contains($id)) ? 'selected' : '' }}>{{ $review }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payments') ? 'is-invalid' : '' }}" name="payments[]" id="payments" multiple>
                    @foreach($payments as $id => $payment)
                        <option value="{{ $id }}" {{ (in_array($id, old('payments', [])) || $serviceDetail->payments->contains($id)) ? 'selected' : '' }}>{{ $payment }}</option>
                    @endforeach
                </select>
